...
const [randomPatrolConfig, setRandomPatrolConfig] = useState([]);
const [randomPatrolHistory, setRandomPatrolHistory] = useState([]);
const [randomSelectedPointIds, setRandomSelectedPointIds] = useState([]);
const [randomPatrolsPerDay, setRandomPatrolsPerDay] = useState("1");
const [randomPatrolEnabled, setRandomPatrolEnabled] = useState(true);
const [randomPatrolSaveStatus, setRandomPatrolSaveStatus] = useState("");
...
const loadRandomPatrolConfig = async (siteId) => {
  if (!siteId) return;
  try {
    const sessionToken = getSessionToken();
    const [configResponse, historyResponse] = await Promise.all([
      fetch(`${API_BASE_URL}/settings/sites/${siteId}/random-patrol-config`, {
        headers: { Authorization: `Bearer ${sessionToken}` },
      }),
      fetch(`${API_BASE_URL}/settings/sites/${siteId}/random-patrol-config/history`, {
        headers: { Authorization: `Bearer ${sessionToken}` },
      }),
    ]);
    const configData = await configResponse.json();
    const historyData = await historyResponse.json();
    if (!configResponse.ok) throw new Error(configData.message || "Unable to load Random Patrol configuration");
    setRandomPatrolConfig(configData.points || []);
    setRandomPatrolHistory(historyResponse.ok ? historyData.history || [] : []);
  } catch (err) {
    console.error("Random Patrol configuration load error", err);
    setRandomPatrolSaveStatus(err.message || "Load failed");
  }
};

const saveRandomPatrolConfig = async () => {
  if (!patrolSite || randomSelectedPointIds.length === 0) {
    setRandomPatrolSaveStatus("Select at least one Patrol Point.");
    return;
  }
  setRandomPatrolSaveStatus("Saving...");
  try {
    const response = await fetch(
      `${API_BASE_URL}/settings/sites/${patrolSite.id}/random-patrol-config`,
      {
        method: "PUT",
        headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
        body: JSON.stringify({
          point_ids: randomSelectedPointIds,
          enabled: randomPatrolEnabled,
          patrols_per_day: Number(randomPatrolsPerDay),
        }),
      }
    );
    const data = await response.json();
      ...
      <body>
        <div class="report-header">
          ...
          <div class="report-meta">
            ...
            Site ID: ${formatValue(site.site_code)}<br/>
...
>
  ...
  loadRandomPatrolConfig(site.id);
}}
>
  Patrols
</button>}
...
{profileSite && (
  ...
  <h2 style={{ marginBottom: "10px" }}>
    {profileSite.site_code} | {profileSite.name}
  </h2>
...
<label className="settings-field">
  <span>Site ID</span>
  <input aria-label="Site ID" value={profileSite.site_code || ""} readOnly />
</label>

<label className="settings-field">
  <span>Location</span>
  <input value={profileSite.location || ""} onChange={(event) =>
    setProfileSite({ ...profileSite, location: event.target.value })
  } />
</label>
...
{patrolSite && (
  <div className="modal-overlay">
    <div className="recipients-modal">
      ...
      <p>
        {patrolSite.site_code} | {patrolSite.name}
      </p>
...
{activePatrolTab === "schedule" && (
  <>
    ...
    <div className="patrol-schedule-box random-patrol-config">
      <h4>Random Daily Patrols</h4>
      <p className="random-patrol-help">
        Select one, several or all Patrol Points. The selected daily count is applied independently to every selected point. Times are generated by the backend at 00:01 and remain hidden from guards until 15 minutes before each patrol.
      </p>

      <div className="random-patrol-points">
        <label className="random-patrol-point select-all">
          <input
            type="checkbox"
            checked={randomPatrolConfig.length > 0 && randomSelectedPointIds.length === randomPatrolConfig.length}
            onChange={(event) => setRandomSelectedPointIds(
              event.target.checked ? randomPatrolConfig.map((point) => Number(point.patrol_point_id)) : []
            )}
          />
          <strong>All active points</strong>
        </label>
        {randomPatrolConfig.map((point) => {
          const pointId = Number(point.patrol_point_id);
          return (
            <label className="random-patrol-point" key={pointId}>
              <input
                type="checkbox"
                checked={randomSelectedPointIds.includes(pointId)}
                onChange={(event) => setRandomSelectedPointIds((current) =>
              ...
              <span>
                ...
                <small>
                  {point.enabled == null
                    ? "Not configured"
                    : `${point.enabled ? "Enabled" : "Disabled"} · ${point.patrols_per_day}/day · effective ${new Date(point.effective_from_date).toLocaleDateString("el-GR")}`}
                </small>
              </span>
            </label>
          );
      ...
      <div className="random-patrol-controls">
        <label className="settings-field">
          <span>Patrols per selected point / 24h</span>
          <input
            type="number"
            min="1"
            max="20"
            value={randomPatrolsPerDay}
            onChange={(event) => setRandomPatrolsPerDay(event.target.value)}
          />
        </label>
        <label className="random-patrol-enabled">
          <input
            type="checkbox"
            checked={randomPatrolEnabled}
            onChange={(event) => setRandomPatrolEnabled(event.target.checked)}
          />
          Enabled for selected points
        </label>
        <button type="button" className="primary-button" onClick={saveRandomPatrolConfig}>
          Apply to Selected Points
        </button>
      </div>

      {randomPatrolSaveStatus && <div className="profile-save-status">{randomPatrolSaveStatus}</div>}

      <details className="random-patrol-history">
        <summary>Configuration Audit History ({randomPatrolHistory.length})</summary>
        {randomPatrolHistory.slice(0, 50).map((entry) => (
          <div key={entry.id} className="random-patrol-history-row">
            <strong>{entry.point_name}</strong>
            <span>
              {new Date(entry.changed_at).toLocaleString("el-GR")} · {entry.admin_username || "Administrator"} · {entry.previous_patrols_per_day ?? "—"} → {entry.new_patrols_per_day} patrols/day · {entry.new_enabled ? "Enabled" : "Disabled"}
            </span>
          </div>
        ))}
      </details>
